...
function StatCard({ num, label, tag }) {
  return (
    <div className='border border-[#e8e8e8] rounded-[10px] p-[18px] hover:border-[#ccc] hover:shadow-sm transition-all'>
      ...
      <div className='mt-2.5 text-[11px] font-semibold text-[#22c55e]'>
        {tag}
      </div>
    </div>
  );
...
function BarItem({ label, count, max, color = "#22c55e" }) {
  ...
  return (
    <div className='flex items-center gap-3'>
      {/* Label */}
      <span
        className='text-[12px] text-[#444] w-48 shrink-0 truncate'
        title={label}
      >
        {label}
      </span>
...
export default function AdminDashboardPage() {
  ...
  return (
    <>
      <AdminTopbar
        title='Dashboard'
        subtitle='Vue générale du système'
      />
      ...
      <div className='p-7 flex flex-col gap-6'>
          ...
          {statsLoading ? (
            ...
            <>
              ...
              <StatCard
                num={stats.matieres}
                label='Matières'
                tag='Avec difficultés'
              />
              <StatCard
                num={stats.ressources}
                label='Ressources'
                tag='↑ disponibles'
              />
            </>
          ...
          <div className='border border-[#e8e8e8] rounded-xl overflow-hidden'>
            <div className='px-[18px] py-3.5 border-b border-[#e8e8e8]'>
              <h3 className='text-[13px] font-semibold'>
                Top difficultés déclarées
              </h3>
              <p className='text-[11px] text-[#888] mt-0.5'>
                {statsSubLoading
                  ? "Chargement…"
                  : `Sur ${allSubs.length} soumissions`}
              </p>
            </div>
        ...
        <div className='border border-[#e8e8e8] rounded-xl overflow-hidden'>
          <div className='px-[18px] py-3.5 border-b border-[#e8e8e8]'>
            ...
            <p className='text-[11px] text-[#888] mt-0.5'>
              Les 10 dernières analyses générées
            </p>
          </div>
          <table className='w-full border-collapse'>
            <thead>
              <tr className='border-b border-[#e8e8e8]'>
                {["Étudiant", "Filière", "Matière", "Date", "Statut"].map(
                  (h) => (
                    <th
                      key={h}
