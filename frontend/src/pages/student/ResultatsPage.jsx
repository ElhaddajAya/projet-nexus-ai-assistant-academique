...
const RESULTATS = [
  {
    ...
    filiere: "4IIR", semestre: "S6",
    date: "06/03/2026",
    score: 72,
    analyse: "Votre profil révèle des lacunes principalement au niveau des relations entre classes, notamment l'agrégation et la composition. Vous maîtrisez les bases de la notation UML mais rencontrez des difficultés à modéliser des cas d'utilisation complexes avec des relations d'inclusion et d'extension.",
    plan: [
      { step: 1, titre: "Révision des relations UML",     duree: "Jours 1–2",  desc: "Revoir agrégation, composition, héritage avec des exemples concrets." },
      { step: 2, titre: "Diagrammes de cas d'utilisation",duree: "Jours 3–4",  desc: "Pratiquer les inclusions, extensions et généralisations." },
      { step: 3, titre: "Exercices pratiques",            duree: "Jours 5–6",  desc: "Réaliser les TP notés pour consolider les acquis." },
      { step: 4, titre: "Révision finale",                duree: "Jour 7",     desc: "Mini-examen blanc + correction." },
    ],
    ressources: [
      { titre: "Cours UML – Conception OO",    type: "doc", source: "EMSI"    },
      { titre: "UML – Héritage et composition", type: "vid", source: "YouTube" },
  ...
  },
  {
    ...
    date: "02/03/2026",
    score: 58,
    analyse: "Votre niveau actuel montre une bonne compréhension des algorithmes de base mais des difficultés à choisir le bon modèle selon le contexte et à interpréter les métriques d'évaluation.",
    plan: [
      { step: 1, titre: "Fondamentaux du ML",    duree: "Jours 1–3", desc: "Régression, classification, clustering." },
      { step: 2, titre: "Métriques d'évaluation",duree: "Jours 4–5", desc: "Accuracy, precision, recall, F1-score." },
      { step: 3, titre: "Sélection de modèles",  duree: "Jours 6–7", desc: "Critères de choix selon le problème." },
      { step: 4, titre: "Projets pratiques",     duree: "Jours 8–10",desc: "Application sur datasets réels." },
    ],
    ressources: [
...
export function ResultatsListPage() {
  ...
  return (
    <>
      <StudentTopbar title="Mes résultats" subtitle="Historique de vos analyses" />
      <div className="p-7">
        <div className="border border-[#e8e8e8] rounded-xl overflow-hidden">
          <div className="px-[18px] py-3.5 border-b border-[#e8e8e8]">
            ...
            <p className="text-[11px] text-[#888] mt-0.5">{RESULTATS.length} analyses générées</p>
          </div>
            ...
            {RESULTATS.map((r) => (
              ...
              >
                <div>
                  ...
                  <p className="text-[11px] text-[#888] mt-0.5">{r.module} · {r.filiere} · {r.semestre}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-[11px] font-mono text-[#888]">{r.date}</span>
                  <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full
                    ${r.score >= 75 ? "bg-[#f0fdf4] text-[#22c55e]" : r.score >= 55 ? "bg-amber-50 text-amber-600" : "bg-red-50 text-red-500"}`}>
                    {r.score}%
                  </span>
                  <svg className="w-3.5 h-3.5 text-[#ccc] group-hover:text-[#888] transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    ...
    <>
      ...
      <div className="p-7 flex flex-col gap-5 max-w-[780px]">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="ml-auto text-[11px] font-mono text-[#888]">{r.date}</span>
          <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full
            ${r.score >= 75 ? "bg-[#f0fdf4] text-[#22c55e]" : r.score >= 55 ? "bg-amber-50 text-amber-600" : "bg-red-50 text-red-500"}`}>
            {r.score}%
          </span>
        ...
        <div className="border border-[#e8e8e8] rounded-xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] font-semibold text-[#888] bg-[#f9f9f9] border border-[#e8e8e8] px-2 py-0.5 rounded-full">Gemini AI</span>
          </div>
          ...
          <div className="mt-3 p-3 bg-[#f9f9f9] border border-[#e8e8e8] rounded-lg text-[12px] text-[#888] leading-relaxed">
            💡 Cette analyse est basée uniquement sur vos réponses et les ressources configurées par votre administration.
          </div>
        </div>

        {/* Plan */}
        <div className="border border-[#e8e8e8] rounded-xl p-5">
          <h3 className="text-[13px] font-semibold mb-4">Plan d'apprentissage</h3>
          <div className="flex flex-col gap-3">
            {r.plan.map((p) => (
              <div key={p.step} className="flex gap-3.5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <strong className="text-[13px] font-semibold">{p.titre}</strong>
                    <span className="text-[10px] font-mono text-[#888] bg-[#f9f9f9] border border-[#e8e8e8] px-1.5 py-0.5 rounded">{p.duree}</span>
                  </div>
                  <p className="text-[12px] text-[#888] mt-0.5 leading-relaxed">{p.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Ressources */}
        <div className="border border-[#e8e8e8] rounded-xl p-5">
          <h3 className="text-[13px] font-semibold mb-3">Ressources recommandées</h3>
          <div className="flex flex-col gap-2">
            {r.ressources.map((res, i) => {
              const t = TYPE_STYLE[res.type];
              return (
                <div key={i} className="flex items-center gap-3 p-3 border border-[#e8e8e8] rounded-lg hover:bg-[#f9f9f9] transition-colors cursor-pointer">
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0 ${t.cls}`}>{t.label}</span>
        ...
        <div className="border border-[#e8e8e8] rounded-xl overflow-hidden">
          <div className="px-5 py-3.5 border-b border-[#e8e8e8]">
            <h3 className="text-[13px] font-semibold">Poser une question à Gemini</h3>
            <p className="text-[11px] text-[#888] mt-0.5">Les réponses sont basées sur votre profil et vos ressources</p>
          </div>
                ...
                <div className="flex justify-start">
                  <div className="bg-[#f9f9f9] border border-[#e8e8e8] px-3.5 py-2.5 rounded-xl rounded-bl-sm">
                    <div className="flex gap-1">
                      {[0,1,2].map((i) => (
                        <div key={i} className="w-1.5 h-1.5 rounded-full bg-[#888] animate-bounce" style={{ animationDelay: `${i*0.15}s` }} />
                      ))}
                    </div>
                  </div>
                </div>
