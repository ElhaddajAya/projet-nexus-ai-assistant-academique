      ...
      try {
        ...
        const enriched = await Promise.all(
          submissions.map(async (sub) => {
            try {
              const { data: reco } = await api.get(`/recommendations/${sub._id}`);
      ...
      <div className="p-7">
        <div className="border border-[#e8e8e8] rounded-xl overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="text-center py-10 text-[13px] text-red-500">{error}</div>
          ) : items.length === 0 ? (
            <div className="flex flex-col items-center gap-3 py-16 text-center">
              <p className="text-[13px] text-[#888]">Aucune analyse pour l'instant.</p>
              <button
            ...
            <>
              <div className="divide-y divide-[#e8e8e8]">
                {paginated.map(({ submission, recommendation }) => (
                  <div
                    key={submission._id}
                    onClick={() => navigate(`/student/resultats/${submission._id}`)}
...
export function ResultatDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setSubmission(sub);
        const { data: reco } = await api.get(`/recommendations/${submissionId}`);
        setRecommendation(reco);
        if (reco.chat_history && reco.chat_history.length > 0) {
          setMessages(reco.chat_history.map((msg) => ({ role: msg.role, text: msg.message })));
        }
      } catch {
  ...
  async function sendQuestion(q) {
    ...
    setChatLoading(true);
    try {
      const { data } = await api.post("/recommendations/ask", { submissionId, question: q });
      setMessages((prev) => [...prev, { role: "ai", text: data.answer }]);
    } catch {
      setMessages((prev) => [...prev, { role: "ai", text: "Désolé, je n'ai pas pu répondre. Réessayez dans quelques instants." }]);
    } finally {
  ...
  if (loading) {
    return (<><StudentTopbar title="Analyse" subtitle="Chargement en cours…" /><Spinner /></>);
  }

  if (error || !recommendation) {
    return (
      <>
        <StudentTopbar title="Analyse" subtitle="" />
        <div className="flex flex-col items-center gap-3 py-20 text-center">
          <p className="text-[13px] text-red-500">{error || "Analyse introuvable."}</p>
          <button
    ...
    <>
      ...
      <div className="p-7 flex flex-col gap-5 w-full">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {[submission?.filiereId?.nom_filiere, submission?.semestre, submission?.moduleId?.nom_module, submission?.matiereId?.nom_matiere]
            .filter(Boolean)
            .map((c, i, arr) => (
              <span key={i} className="flex items-center gap-1">
                <span className="text-[12px] text-[#888]">{c}</span>
                {i < arr.length - 1 && <span className="text-[#ccc] text-[10px]">›</span>}
              </span>
            ))}
          <span className="ml-auto text-[11px] font-mono text-[#888]">{formatDate(recommendation.createdAt)}</span>
        </div>
        ...
        <div className="border border-[#e8e8e8] rounded-xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] font-semibold text-[#888] bg-[#f9f9f9] border border-[#e8e8e8] px-2 py-0.5 rounded-full">Groq AI</span>
          </div>
          <p className="text-[13px] text-[#444] leading-relaxed">{recommendation.analyse}</p>
          <div className="mt-3 p-3 bg-[#f9f9f9] border border-[#e8e8e8] rounded-lg text-[12px] text-[#888] leading-relaxed">
            💡 Cette analyse est basée sur vos réponses et les ressources configurées par votre administration.
          </div>
        </div>
        ...
        {recommendation.conseils_ia?.length > 0 && (
          <div className="border border-[#e8e8e8] rounded-xl p-5">
            <h3 className="text-[13px] font-semibold mb-3">Conseils personnalisés</h3>
            <ul className="flex flex-col gap-2">
              {recommendation.conseils_ia.map((conseil, i) => (
                <li key={i} className="flex gap-2.5 text-[13px] text-[#444] leading-relaxed">
          ...
          <div className="border border-[#e8e8e8] rounded-xl p-5">
            ...
            <div className="flex flex-col gap-2">
              {recommendation.ressources_recommandees.map((res, i) => {
                ...
                return (
                  <a key={i} href={res.lien} target="_blank" rel="noreferrer"
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className="text-[13px] font-medium text-[#111]">{res.titre}</p>
                      <p className="text-[11px] text-[#888] truncate">{res.lien}</p>
                    </div>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="2" className="group-hover:stroke-[#888] transition-colors shrink-0">
        ...
        <div className="border border-[#e8e8e8] rounded-xl overflow-hidden">
          <div className="px-5 py-3.5 border-b border-[#e8e8e8]">
            <h3 className="text-[13px] font-semibold">Poser une question à Groq AI</h3>
            <p className="text-[11px] text-[#888] mt-0.5">Les réponses sont basées sur votre profil et votre plan</p>
          </div>
