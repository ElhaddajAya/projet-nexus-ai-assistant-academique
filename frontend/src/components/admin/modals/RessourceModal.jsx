...
export default function RessourceModal({ open, onClose, onSave, initial }) {
  ...
  useEffect(() => {
    if (initial) {
      ...
      setTitre(""); setType(""); setFiliereId("");
      setNiveau(""); setMatiereId(""); setLien("");
    }
  }, [initial, open]);
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Filière">
          <select
            ...
            value={filiereId}
            onChange={(e) => { setFiliereId(e.target.value); setMatiereId(""); }}
          >
            <option value="" disabled>Choisir…</option>
            {filieres.map((f) => (
              <option key={f._id} value={f._id}>{f.nom_filiere}</option>
            ))}
          </select>
        </Field>
      ...
      <Field label="Matière">
        ...
        >
          <option value="" disabled>
            {filiereId ? "Choisir une matière…" : "Sélectionnez d'abord une filière…"}
          </option>
          {matieres.map((m) => (
