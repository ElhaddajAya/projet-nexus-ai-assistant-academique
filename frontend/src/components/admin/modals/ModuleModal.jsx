...
export default function ModuleModal({ open, onClose, onSave, initial }) {
  ...
  useEffect(() => {
    if (initial) { setNom(initial.nom); setFiliere(initial.filiere); setSemestre(initial.semestre); }
    else         { setNom(""); setFiliere(""); setSemestre(""); }
  }, [initial, open]);
  ...
  return (
    ...
    >
      <Field label="Nom du module">
        <input className={inputClass} value={nom} onChange={(e) => setNom(e.target.value)} placeholder="Ex : Intelligence Artificielle" />
      </Field>

      <div className="grid grid-cols-2 gap-3">
        <Field label="Filière">
          <select className={selectClass} value={filiere} onChange={(e) => setFiliere(e.target.value)}>
